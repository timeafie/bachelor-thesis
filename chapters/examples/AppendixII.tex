...
\[
\begin{split}
    \Phi^{\mathsf{st}}_{F_{\leftrightarrow}}
...
Implikation wird als \(\neg A\lor B\) ausgegeben. Dadurch entsteht die
folgende vollständige QuAbS-Eingabe:
...
Zunächst wird ein vorhandenes Zielverzeichnis für die QCIR-Dateien
angelegt:
\begin{lstlisting}[language={}]
mkdir -p out
\end{lstlisting}
Für \(X=\{a_1\}\), \(Y=\{a_2\}\) und \(Z=\emptyset\) können anschließend
die drei Kodierungen erzeugt werden:
\begin{lstlisting}[
    language={},
...
\begin{cases}
\lfloor n/5\rfloor,
    &\text{für }\mathsf{st}\text{ und }\mathsf{co},\\
\lfloor n/10\rfloor,
    &\text{für }\mathsf{pr}.
\end{cases}
\]
...
\end{lstlisting}
Für einen kurzen Funktionstest mit einer einzelnen kleinen Instanz
...
\begin{table}[htb]
    ...
    \begin{tabularx}{\textwidth}{@{}lcX@{}}
        ...
        \texttt{LIMIT} & \(0\)
            & Maximale Zahl von AF-Dateien; \(0\) bedeutet ohne
              Begrenzung.\\
        \texttt{NUM\_PAIRS} & \(5\)
...
\end{description}
Die wichtigsten Statuswerte besitzen folgende Bedeutung:
\begin{table}[htb]
    \centering
    \begin{tabularx}{\textwidth}{@{}lcX@{}}
        \toprule
