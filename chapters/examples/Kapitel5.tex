...
wirken sich unterschiedlich auf die betrachteten Semantiken aus. 
Dieses Kapitel untersucht deshalb, wie sich der Ausschluss bestimmter Zyklen 
auf die bedingte Unabhängigkeit und auf die Komplexität ihrer Entscheidung auswirkt.

Zunächst werden in Abschnitt~\ref{sec:acyc-afs} azyklische AFs betrachtet. 
Da ihre Struktur die semantische Bewertung für \(\sigma\in\{co,st,pr\}\) 
...
aus der Übereinstimmung beider Semantiken auf dieser Graphklasse. Für die \emph{complete}-
Semantik wird eine neue Konstruktion entwickelt, welche die zusätzlich möglichen 
partiellen Labellings kontrolliert. 
Damit wird für alle drei Semantiken \(\Pi_2^\text{P}\)-Vollständigkeit nachgewiesen.

\section{Azyklische AFs}
\label{sec:acyc-afs}
Ein AF \(F=(A,R)\) heißt azyklisch, wenn der gerichtete Graph \((A,R)\) keinen gerichteten Zyklus enthält. 
Azyklische AFs bilden eine besonders restriktive Graphklasse, da sämtliche zyklischen Angriffsbeziehungen
 ausgeschlossen sind. Im Folgenden wird gezeigt, dass diese strukturelle Einschränkung die möglichen Labellings 
vollständig festlegt und damit eine unmittelbare Aussage über \(X\perp_\sigma Y\mid Z\) erlaubt.
\begin{Proposition}
\label{prop:acyc-af-ci-st-top}
    Sei \(F=(A,R)\) ein azyklisches AF und \(\sigma\) eine Semantik mit
    \(\sigma \in \{co,st,pr\}\). Dann gilt für alle paarweise disjunkten
...
\label{pr:acyc-af-ci-st-top-true}
    ...
    Da das \emph{grounded}-Labelling eindeutig ist, existiert genau ein Labelling \(\lambda\),
     sodass \(\Lambda_{co}(F)=\Lambda_{pr}(F)=\Lambda_{st}(F)=\{\lambda\}\).
    Sei nun \(\sigma \in \{co,st,pr\}\). Dann gilt insbesondere \(|\Lambda_\sigma(F)|=1\).
...
Damit beruht die bedingte Unabhängigkeit nicht auf einer Separation von \(X\) und \(Y\), 
sondern auf der fehlenden Variation innerhalb von \(\Lambda_\sigma(F)\).
...
Dennoch begrenzt das Fehlen gerader Zyklen die Menge der möglichen Labellings so stark, 
...
Auch in AFs ohne Zykel gerader Länge beruht die Allgmeingültigkeit der 
...
semantischer Alternativen. Mit den ungeradezykelfreien AFs ändert sich diese Ausgangslage: 
Zyklen bleiben nun zulässig, sofern sie eine gerade Länge besitzen. Damit bleiben gerade 
jene Rückkopplungen erhalten, die unterschiedliche Labellings ermöglichen können. 

Dennoch ist die Struktur ungeradezykelfreier AFs nicht beliebig. Insbesondere fallen auf dieser 
Graphklasse die \emph{stable}- und die \emph{preferred}-Semantik zusammen. 
Die folgenden Ergebnisse zeigen, dass die strukturelle Einschränkung keine entsprechende Verringerung 
der Komplexität bewirkt. Während sich dies für die \emph{stable}- und die \emph{preferred}-Semantik 
aus bekannten Reduktion von \textcite{bluemel:2025:independence-aba} und ihrer Graphstruktur ableiten lässt, 
...
\begin{proof}
    ...
    Zugehörigkeits- und Härtebeweis wurden zuerst von \textcite{bluemel:2025:independence-aba} bewiesen. 
    Es bleibt zu zeigen, dass es sich bei der Konstruktion um einen ungeradezykelfreien Graphen handelt.
...
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{
        \input{gfx/Blümel_Reduktion.tex}
    }
    \caption{AF Repräsentation der Reduktion in Proposition C.1 für eine Formel 
    \(\forall X \exists Y \varphi\) wo \(\phi\) durch die folgenden Klauseln gegeben ist: \(\{\{x_0, x_2, y_1\}, \{x_0, \overline{x_2}, \overline{y_1}, \overline{x_1}\},
    \{x_0, \overline{x_2}, x_1\}\}\), adaptiert von \cite{bluemel:2025:independence-aba}.}
    \label{fig:reduktion-stable}
\end{figure}
Abbildung~\ref{fig:reduktion-stable} zeigt eine konkrete Instanz der Reduktion.

Das vorangegangene Ergebnis lässt sich auf ungeradezykelfreien AFs unmittelbar auf die \emph{preferred}-Unabhängigkeit übertragen. 
Maßgeblich ist die Gleichheit \(\Lambda_{st}(F)=\Lambda_{pr}(F)\), 
...
da \(\Lambda_{co}(F)\) zusätzliche, nichtmaximale Labellings enthalten kann. 
Insbesondere können Variablenpaare mit \(und\) gelabelt sein, weshalb eine 
...
\begin{proof}
    ...
    Reduziert wird vom \(\Pi_2^P\)-vollständigen Problem
...
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{
        \input{gfx/ocf_co_härte.tex}
    }
    \caption{AF-Repräsentation der Reduktion in \ref{prop:ocf-af-ci-co-pi^2-c} für die Formel
        \(\Psi = \forall x_1 \exists y_1 \varphi\) mit
        \(\varphi =
        (x_1 \lor y_1) \land
        (\overline{x_1} \lor \overline{y_1})\).
        Die reduzierte Formel \(\widehat{\varphi}\) ist durch die Klauseln
        \(\{\{x_0,x_1,y_1\},
...
Abbildung~\ref{fig:reduktion-complete} verdeutlicht, wie die Kontrollstruktur aus \(d_i\), 
\(s\) und \(g\) totale von partiellen Belegungen trennt. Die Konstruktion berücksichtigt damit 
die zusätzlichen \emph{complete}-Labellings, während alle gerichteten Zyklen weiterhin auf 
die Zweierzyklen innerhalb der Variablenpaare und der Komponente \(\{\varphi,t\}\) beschränkt bleiben.
