...
\(\sigma\in\{\mathrm{co},\mathrm{st},\mathrm{pr}\}\) 
...
\label{prop:acyc-af-ci-st-top}
    ...
    \(\sigma \in \{\mathrm{co},\mathrm{st},\mathrm{pr}\}\). Dann gilt für alle paarweise disjunkten
...
\label{pr:acyc-af-ci-st-top-true}
    ...
    Sei nun \(\sigma \in \{\mathrm{co},\mathrm{st},\mathrm{pr}\}\). Dann gilt insbesondere \(|\Lambda_{\sigma}(F)|=1\).
...
\label{even-cycle-free-af-ci-top-true}
    ...
    Somit gilt für jedes \(\sigma \in \{\mathrm{co},\mathrm{st},\mathrm{pr}\}\), dass \(|\Lambda_{\sigma}(F)| \leq 1\). 
...
Abbildung~\ref{fig:reduktion-stable} konkretisiert die im Beweis verwendete
Konstruktion. 
