...
Graphklasse die \emph{stable}- und die \emph{preferred}-Semantik zusammen \cite{DUNNE2002187}.
...
Wir zeigen zuerst, dass sich für die \emph{stable}-Semantik keine Vereinfachung ergibt.
\begin{Proposition}
...
\begin{proof}
    ...
    Sei \(\lambda\in\Lambda_{co}(F_{\Psi})\) ein beliebiges
    ...
    belegt; im dritten Fall ist sie unentschieden. Da \(d_i\) von beiden
    Literalargumenten angegriffen wird, gilt \(\lambda(d_i)=out\) genau
    im totalen Fall und \(\lambda(d_i)=und\) im unentschiedenen Fall.
    Insbesondere erhält kein \(d_i\) das Label \(in\). Folglich gilt
    \(\lambda(s)=in\) genau dann, wenn alle Variablenpaare für \(X'\)
    total gelabelt sind; andernfalls ist \(\lambda(s)=und\). Da \(s\)
    der einzige Angreifer von \(g\) ist, gilt entsprechend
    \(\lambda(g)=out\) beziehungsweise \(\lambda(g)=und\). Im
    partiellen Fall gilt also \(\lambda(g)=und\). Da \(g\) jedes
    \(c_k\) und \(\varphi\) angreift, kann keines dieser Argumente mit
    \(in\) gelabelt sein. Auf diese Weise wird die eigentliche
    ...
    übernimmt.
    Falls \(\lambda_1(s)=\lambda_2(s)=und\), folgt aus den
    Vorbeobachtungen, dass \(\lambda_1(g)=\lambda_2(g)=und\) gilt.
    Da \(g\) das Argument \(\varphi\) angreift, kann \(\varphi\) in
    \(\lambda_1\) nicht mit \(in\) gelabelt sein: Dafür müssten alle
    Angreifer von \(\varphi\) mit \(out\) gelabelt sein. Für jedes
    \(x_i\in X'\) übernehmen wir die Labels des Variablenpaars
    \(x_i,\overline{x_i}\) aus \(\lambda_2\); jedes Variablenpaar
    \(y_j,\overline{y_j}\) labeln wir mit \((und,und)\). Die dadurch
    bestimmten Labels der Argumente \(d_i\) übernehmen wir ebenfalls;
    ferner setzen wir \(\lambda_3(s)=\lambda_3(g)=und\). Jedes
    Klauselargument \(c_k\), das von einem mit \(in\) gelabelten
    Literal angegriffen wird, erhält in \(\lambda_3\) das Label \(out\);
    alle übrigen Klauselargumente erhalten das Label \(und\). Wegen
    \(\lambda_3(g)=und\) sind genau diese beiden Fälle möglich.
    Abschließend wählen wir
    \(\lambda_3(\varphi)=out\) und \(\lambda_3(t)=in\), falls
    \(\lambda_1(\varphi)=out\) gilt. Falls
    \(\lambda_1(\varphi)=und\) gilt, wählen wir dagegen
    \(\lambda_3(\varphi)=\lambda_3(t)=und\). Damit übernimmt
    ...
    \(out\), das Argument \(s\) mit \(in\) und das Argument \(g\) mit
    \(out\) gelabelt. Außerdem wird jedes
    ...
    \(
        ...
        \{(in,out),(out,in),(und,und)\}
    \)
    realisiert werden. Wir wählen dasjenige Paar, für das
    ...
    Belegungen. Dadurch sind alle \(d_i\) mit \(out\), das Argument
    \(s\) mit \(in\) und das Argument \(g\) mit \(out\) gelabelt. Ein
    Klauselargument erhält das Label \(out\), wenn die zugehörige
    Klausel erfüllt ist, und andernfalls das Label \(in\). Da
    \(x_0=\bot\) gilt und keine Belegung von \(Y\) die Formel
    \(\varphi\) unter \(\alpha\) erfüllt, ist mindestens ein
    Klauselargument mit \(in\) gelabelt. Daher gilt notwendigerweise
    \(\lambda_2(\varphi)=out\) und folglich \(\lambda_2(t)=in\).
    Die so festgelegten Labels bilden ein \emph{complete}-Labelling,
    das \(\alpha'\) kodiert und für das \(\lambda_2(s)=in\) gilt.
    Separat konstruieren wir ein \emph{complete}-Labelling \(\lambda_1\)
    mit \(\lambda_1(s)=in\) und \(\lambda_1(\varphi)=in\). Dazu wählen
    wir eine totale Belegung der Variablen aus \(X'\cup Y\) mit
    \(x_0=\top\) und labeln die Variablenpaare entsprechend. Das
    Argument \(x_0\) ist dann mit \(in\) gelabelt und greift alle
    Klauselargumente an, sodass diese das Label \(out\) erhalten. Weil
    alle Variablenpaare total gelabelt sind, gilt außerdem
    \(\lambda_1(s)=in\) und \(\lambda_1(g)=out\). Schließlich setzen
    wir \(\lambda_1(\varphi)=in\) und \(\lambda_1(t)=out\).

    Angenommen, es gäbe ein \emph{complete}-Labelling \(\lambda_3\),
    für das \(\lambda_3(\varphi)=\lambda_1(\varphi)=in\),
    \(\lambda_3|_{X'}=\lambda_2|_{X'}\) und
    \(\lambda_3(s)=\lambda_1(s)=\lambda_2(s)=in\) gilt. Aus
    \(\lambda_3(\varphi)=in\) folgt, dass alle Klauselargumente \(c_k\)
    mit \(out\) gelabelt sind. Wegen \(\lambda_3(s)=in\) gilt auch
    \(\lambda_3(g)=out\). Daher muss
    jedes \(c_k\) von einem mit \(in\) gelabelten Literalargument
    angegriffen werden. Die mit \(in\) gelabelten \(Y\)-Literale
...
\begin{figure}
    ...
    \caption{AF-Repräsentation der Reduktion aus Proposition~\ref{prop:ocf-af-ci-co-pi^2-c} für die Formel
