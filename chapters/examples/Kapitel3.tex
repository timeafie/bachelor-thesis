...
\begin{Example}
    ...
    \[
        ...
        \lambda_3|_Z=\lambda_1|_Z=\lambda_2|_Z.
    \]
    Diese Rekombination ist bei allen \emph{complete}-Labellings möglich,
    die auf \(Z\) übereinstimmen. Sind \(m\) mit
...
\begin{Definition}[\cite{bluemel:2025:independence-aba}]
    ...
    \[
        \begin{array}{ll}
        \text{Eingabe:}
            &\text{AF } F=(A,R)\text{ sowie paarweise disjunkte Mengen }
             X,Y,Z\subseteq A\\
        \text{Frage:}
...
\(\Pi_3^{\mathrm{P}}\)-vollständig.
\end{Theorem}
Die Komplexität entsteht durch die Betrachtung aller auf \(Z\) übereinstimmenden Labellingpaare, 
...
durch die Existenz genau eines oder keines Labellings entstehen.
...
\begin{Definition}
    ...
    Sei \(F=(A,R)\) ein AF und seien \(\mathbf V\) und \(\widehat{\mathbf V}\) Labellingvariablenblöcke.
    \[
...
\begin{proof}
    ...
    \(\operatorname{Co}_F\) genau die Definition eines \emph{complete}-Labellings wieder. 
    Für \(\sigma=\mathsf{st}\) kommt die Bedingung
    \(\mathsf{und}(\lambda_\nu)=\emptyset\) hinzu.
...
\begin{Definition}
    ...
    Sei \(F =(A,R)\) ein AF, \(S\subseteq A\) und seien \(\mathbf V^j\) und \(\mathbf V^k\) zwei Kopien eines
...
\label{def:eq-mix-formula}
    Sei \(F =(A,R)\) ein AF. Seien \(X,Y,Z\subseteq A\) paarweise disjunkte Mengen, 
...
\begin{Definition}
    ...
    Sei \(F=(A,R)\) ein AF, seien \(X,Y,Z\subseteq A\) paarweise disjunkt, \(\mathbf V^1, 
    \mathbf V^2, \mathbf V^3\) paarweise disjunkte Labellingvariablenblöcke und
...
gewählten Labellings \(\lambda_1\) und \(\lambda_2\). Der existenzielle
...
\begin{Theorem}
    ...
    Für jedes \(\text{AF }F=(A,R)\), alle paarweise disjunkten Mengen
