...
Ein Argumentationssystem kann bezüglich einer Semantik mehrere gültige Labellings besitzen, 
zwischen deren Bewertungen Abhängigkeiten bestehen. Die Kenntnis solcher Abhängigkeiten erlaubt es, 
semantisch voneinander trennbare Bereiche zu erkennen und gegebenenfalls unabhängig zu untersuchen. 
Seien dazu \(F=(A,R)\) ein Argumentationssystem, \(\sigma\) eine Semantik und \(X,Y,Z\subseteq A\) 
paarweise disjunkte Mengen von Argumenten. Intuitiv gilt \(X\perp_\sigma Y\mid Z\) in \(F\), 
wenn sich die Bewertung von \(X\) aus einem gültigen Labelling und die Bewertung von \(Y\) aus 
einem zweiten gültigen Labelling kombinieren lassen, sofern beide auf \(Z\) übereinstimmen. 
Der zugrunde liegende semantische Unabhängigkeitsbegriff geht auf \citeauthor*{rienstra:2020:independence-af}~\cite{rienstra:2020:independence-af} zurück.

Die Entscheidung, ob \(X\perp_\sigma Y\mid Z\) gilt, erfordert die Betrachtung aller auf \(Z\) 
übereinstimmenden Labelling-Paare und die Suche nach einem geeigneten dritten Labelling. 
Entsprechend ist ihre Berechnung bereits für die hier betrachteten Standardsemantiken schwierig: 
für die \emph{complete}- und die \emph{stable}-Semantik sind die zugehörigen Entscheidungsprobleme 
\(\Pi_2^{\mathrm P}\)-vollständig, während das Problem für die \emph{preferred}-Semantik
\(\Pi_3^{\mathrm P}\)-vollständig ist \cite{bluemel:2025:independence-aba}. 
Die Quantorenstruktur dieser Komplexitätsklassen motiviert 
QBFs als Zielformalismus. Ihr Einsatz zur Kodierung von Argumentationsproblemen wurde 
insbesondere von \citeauthor*{egly:woltran:2006:reasoning-qbf}~\cite{egly:woltran:2006:reasoning-qbf} untersucht.

Ziel dieses Kapitels ist die Entwicklung einer einheitlichen, labellingbasierten 
QBF-Kodierung der bedingten Unabhängigkeit für die drei betrachteten Semantiken. 
Zugleich werden Sonderfälle berücksichtigt, 
in denen ein Argumentationssystem bezüglich der betrachteten Semantik höchstens ein Labelling 
besitzt und die Unabhängigkeitsbedingung daher trivial erfüllt ist.

Das Kapitel ist in drei Schritte gegliedert. Zunächst werden der semantische 
Unabhängigkeitsbegriff, das zugehörige Entscheidungsproblem und dessen Komplexität 
eingeführt. Anschließend werden boolesche Variablenblöcke zur Repräsentation von 
Labellings sowie Kodierungen der \emph{complete}-, \emph{stable}- 
und \emph{preferred}-Semantik entwickelt. Im letzten Schritt werden Formeln für die 
Gleichheit eingeschränkter Labellings und ihre Rekombination definiert. 
Gemeinsam mit den Semantikkodierungen werden sie gemäß der Quantorenstruktur 
der Unabhängigkeitsdefinition zur QBF \(\Phi_F^\sigma(X,Y\mid Z)\) zusammengesetzt, 
deren Korrektheit abschließend bewiesen wird.
...
Dazu werden zunächst der semantische Unabhängigkeitsbegriff und das zugehörige Entscheidungsproblem eingeführt. 
...
Allgemein bezeichnen wir mit \(\mathbf V^k=(I^k,O^k,U^k)\) die
\emph{\(k\)-te Kopie} eines Variablenblocks. 
...
\begin{Definition}
    ...
    \mathbf V^2, \mathbf V^3\) Labellingvariablenblöcke und
...
Das Ergebnis zeigt, dass die bedingte Unabhängigkeit vollständig 
durch die Wahrheit von \(\Phi_F^\sigma(X,Y\mid Z)\) charakterisiert wird.
Damit ist die Grundlage für die algorithmische Umsetzung und 
die anschließende empirische Untersuchung geschaffen.