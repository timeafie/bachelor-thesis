...
Bedingte Unabhängigkeit untersucht, ob sich die Bewertungen der Mengen \(X\) und 
\(Y\) aus verschiedenen gültigen Labellings bei festgehaltener Bewertung von \(Z\) 
in einem dritten gültigen Labelling kombinieren lassen. Die zugrunde liegende
 semantische Definition geht auf \citeauthor*{rienstra:2020:independence-af}\cite{rienstra:2020:independence-af} zurück. 
 Notation und Komplexitätsergebnisse orientieren sich an 
\citeauthor*{bluemel:2025:independence-aba} \cite{bluemel:2025:independence-aba}: Für die \emph{complete}- und die 
 \emph{stable}-Semantik ist das Entscheidungsproblem \(\Pi_2^{\mathrm P}\)-vollständig, 
 für die \emph{preferred}-Semantik hingegen \(\Pi_3^{\mathrm P}\)-vollständig.
  Diese Ergebnisse motivieren quantifizierte boolesche Formeln als Zielformalismus, 
deren Einsatz für Argumentationsprobleme insbesondere von 
\citeauthor*{egly:woltran:2006:reasoning-qbf}\cite{egly:woltran:2006:reasoning-qbf} untersucht wurde.
...
Hierzu werden die semantische Gültigkeit einzelner Labellings und ihre für die 
Unabhängigkeit erforderliche Rekombination zunächst als getrennte Bausteine kodiert. 
Diese Trennung ermöglicht es, dieselben Formeln für Übereinstimmung und Rekombination 
unabhängig von der gewählten Semantik zu verwenden. Zugleich werden Sonderfälle berücksichtigt, 
...
Dieses Unterkapitel untersucht bedingte Unabhängigkeit in Argumentationssystemen und 
bereitet ihre Kodierung durch quantifizierte boolesche Formeln vor.
Dazu werden zunächst der semantische Unabhängigkeitsbegriff und das zugehörige 
Entscheidungsproblem eingeführt. Anschließend werden die relevanten 
Komplexitätsergebnisse und jene Sonderfälle betrachtet, die für die Konstruktion 
und empirische Auswertung der QBF-Kodierungen von Bedeutung sind.
...
\begin{Definition}[\cite{bluemel:2025:independence-aba}]
    ...
    \]
\end{Definition}
Die Problemformulierung macht zugleich die für die spätere QBF-Kodierung maßgebliche Quantorenstruktur sichtbar: 
Abhängigkeit liegt genau dann vor, wenn ein auf \(Z\) übereinstimmendes Paar von \(\sigma\)-Labellings existiert, 
für das kein geeignetes rekombinierendes \(\sigma\)-Labelling gefunden werden kann.
...
Intuitiv wählt eine Belegung, die \(\operatorname{One}_F(\mathbf V)\) erfüllt, für jedes Argument genau eine der drei Labelvariablen \(i_a\), \(o_a\) und \(u_a\) aus. 
