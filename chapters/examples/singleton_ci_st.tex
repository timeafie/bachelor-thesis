...
\(X=\{x\}\), \(Y=\{y\}\) und \(Z\) paarweise disjunkt. Zu entscheiden ist, ob
\[
...
Sei \(F=(A,R)\) ein AF, seien \(X=\{x\}\), \(Y=\{y\}\) mit \(x\neq y\) und sei
\(Z\subseteq A\setminus\{x,y\}\). Für \(k\in\{1,2,3\}\) bezeichne
\(\mathcal L^k\) eine Kopie der Labelling-Variablen, und für ein Argument
\(a\in A\) sowie ein Label \(\ell\in L_{\mathrm{st}}\) schreibe
\(\mathrm{Fix}^k_a(\ell)\) für die Formel, die \(a\) in der \(k\)-ten Kopie auf
\(\ell\) festlegt.

Alle Anfragen werden als Erfüllbarkeitstests auf der Kodierung
\(\mathrm{st}_F\) aus Definition~\ref{def:st-pl} mit fixierten Assumptions
gestellt. Wir fassen die wiederkehrende Grundabfrage zusammen: Für eine
Teilbelegung \(\pi:W\to L_{\mathrm{st}}\) mit \(W\subseteq A\) sei
\[
    \mathrm{realisierbar}(\pi)
    \quad:\Longleftrightarrow\quad
    \mathrm{st}_F(\mathcal L^3)\land
    \bigwedge_{a\in W}\mathrm{Fix}^3_a(\pi(a))
    \text{ ist erfüllbar}.
\]
...
existiert; einzig die SAT/UNSAT-Antwort wird benötigt. Insbesondere ist
\(\mathrm{realisierbar}(\emptyset)\) genau dann wahr, wenn
\(\Lambda_{\mathrm{st}}(F)\neq\emptyset\). Da alle Anfragen dieselbe
Basisformel teilen und sich nur in Assumptions sowie hinzugefügten
Blocking-Klauseln unterscheiden, lassen sie sich inkrementell mit einem
einzigen SAT-Solver beantworten.
