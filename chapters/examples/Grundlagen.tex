...
Der \emph{labelling}-basierte Ansatz ordnet jedem Argument ein eindeutiges Labelling zu.
Im Rahmen dieser Arbeit verwenden wir die \emph{Labels}: \emph{in, out} und \emph{und}. Wobei \emph{in}
die Akzeptanz und \emph{out} den Ausschluss eines Arguments ausdrückt. Das Label \emph{und} erhält
ein Argument, wenn es weder einen Grund gibt es zu akzeptieren noch es zu verwerfen. 

Eine \emph{labelling}-basierte Semantik ist dann eine Funktion, die Argumenten ein \emph{Labelling} 
zuordnet, um schlussendlich eine \enquote{überzeugende} Auswahl an Argumenten mit dem \emph{in-Label} zu erhalten. 

\begin{Definition}
      \label{def:labelling}
      Sei $F = (A, R)$ ein AF. Ein \emph{Labelling} von $F$ ist eine Funktion
      $\lambda : A \rightarrow\{\text{\emph{in, out, und}}\}$, die jedem Argument
      genau eines von drei Labeln zuordnet: \emph{in, out} oder \emph{und}.
\end{Definition}

\begin{Definition}
      \label{def:labelling-set-of-all}
      Sei $F = (A,R)$ ein AF und $\sigma$ eine \emph{labelling}-basierte Semantik.
      Für die Menge aller $\sigma$-\emph{Labellings} von $F$ schreiben wir $\varLambda_{\sigma}(F)$
\end{Definition}

\begin{Definition}
      \label{def:labelling-rest}
      Sei $F=(A,R)$ ein AF, $X \subseteq A$ und $\lambda$ ein beliebiges \emph{Labelling} von $F$.
      Mit $\lambda|_X$ beschreiben wir die Restriktion von $\lambda$ auf $X$.
\end{Definition}


\begin{Definition}
      \label{def:labelling-set}
      Sei $F=(A,R)$ ein AF und $\lambda$ ein \emph{Labelling} von $F$ mit der Menge an \emph{Labels}
      $\mathit{L}=\{\text{\emph{in, out, und}}\}$.
      Dann ist für $lab \in \mathit{L}$, $lab(\lambda) = \{a \in A\mid \lambda(a) = lab\}$.
\end{Definition}
