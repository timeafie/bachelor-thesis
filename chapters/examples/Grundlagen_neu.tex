...
\begin{Definition}
    ...
    \)
    die von \(a\) angegriffenen beziehungsweise \(a\) angreifenden Argumente. 
    Sei \(S\subseteq A\). Für Mengen schreiben wir
    \(
...
\begin{Definition}
    ...
    Sei \(F=(A,R)\) ein Argumentationssystem und sei \(\Lambda_{\mathrm{co}}(F)\) die Menge seiner vollständigen Labellings.  
    Die Grounded-Semantik ist durch \(\Lambda_{\mathrm{gr}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\nexists\lambda'\in\Lambda_{\mathrm{co}}(F):\mathsf{und}(\lambda)\subsetneq\mathsf{und}(\lambda')\}\) definiert. 
    Die Preferred-Semantik ist durch \(\Lambda_{\mathrm{pr}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\nexists\lambda'\in\Lambda_{\mathrm{co}}(F):\mathsf{in}(\lambda)\subsetneq\mathsf{in}(\lambda')\}\) definiert. 
    Die Stable-Semantik ist durch \(\Lambda_{\mathrm{st}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\mathsf{und}(\lambda)=\emptyset\}\) definiert.
\end{Definition}
...
\begin{Definition}
    ...
    Die \emph{Restriktion} von \(\lambda\) auf \(X\) ist 
    \(
        \lambda|_X
    \).
\end{Definition}
...
Die folgenden Definitionen folgen \cite{handbook-satisfiability-2nd}.

\begin{Definition}[QBF]
    \label{def:qbf}
    Die Menge der \emph{quantifizierten booleschen Formeln} ist induktiv definiert als:
    \begin{enumerate}
        \item Aussagenlogische Formeln und die booleschen Konstanten 1 (wahr) und 0 (falsch) sind quantifizierte boolesche Formeln.
        \item Ist \(\varPhi\) eine quantifizierte boolesche Formel, so sind \(\exists x \varPhi\) und \(\forall x \varPhi\) quantifizierte boolesche Formeln
        \item Sind \(\varPhi_1\) und \(\varPhi_2\) quantifizierte boolesche Formel, so sind \(\neg \varPhi_1\), \(\varPhi_1 \lor \varPhi_2\) und \(\varPhi_1 \land \varPhi_2\).
        \item Keine weitere Formel neben (1)-(3) beschreibt quantifizierte boolesche Formeln.
...
\begin{Definition}
    ...
    Sei \(\Phi\) eine quantifizierte boolesche Formel mit den freien
...
beschränkt sich auf die im weiteren Verlauf ben\"otigten Begriffe.
...
\begin{Definition}[Entscheidungsproblem]
    ...
    Länge der Eingabe \(x\).
\end{Definition}
HIER Turingmaschinen EINFÜHREN (OHNE DEFINITION?)
\begin{Definition}[P]
    \label{def:p-np-conp}
...
\begin{Definition}[Orakel]
    Ein \emph{\(\mathcal{C}\)-Orakel} entscheidet für ein Problem
    \(Q\in\mathcal{C}\) in einem Berechnungsschritt, ob eine vorgelegte
    Instanz \(x\) zu \(Q\) gehört. Mit
    \(\mathrm{P}^{\mathcal{C}}\) beziehungsweise
    \(\mathrm{NP}^{\mathcal{C}}\) wird die Klasse der Probleme
    bezeichnet, die eine deterministische beziehungsweise
    nichtdeterministische Turingmaschine in polynomieller Zeit mit
    Zugriff auf ein \(\mathcal{C}\)-Orakel entscheidet.

\end{Definition}

\begin{Definition}[Polynomielle Hierarchie]
    \label{def:polynomial-hierarchy}
    Die Klassen der \emph{polynomialen Hierarchie} sind für
    ...
    Die polynomiale Hierarchie ist die Klasse
...
\begin{Definition}[Polynomielle Reduktion]
    ...
    heißt eine \emph{polynomielle Many-one-Reduktion} von \(Q'\) auf
    \(Q\).
\end{Definition}
...
\begin{Theorem}[\parencites{meyer:stockmeyer:1972:equivalence-regex}
    {stockmeyer:meyer:1973:word-problems}QBF-Vollständigkeit]
    ...
    \(\Pi_k^{\mathrm{P}}\)-vollständig bezüglich polynomialer
    Reduktionen.
\end{Theorem}
