...
\begin{Definition}
    ...
    Sei \(F = (A,R)\) ein AF, \(a \in A\) und \(S \in A\).     
    Mit
    \(
...
\begin{Definition}
    ...
    Die Menge der \emph{grounded}-Semantiken von F ist durch \(\Lambda_{\mathrm{gr}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\nexists\lambda'\in\Lambda_{\mathrm{co}}(F):\mathsf{und}(\lambda)\subsetneq\mathsf{und}(\lambda')\}\) definiert. 
    Die Menge der \emph{preferred}-Semantiken von F ist durch \(\Lambda_{\mathrm{pr}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\nexists\lambda'\in\Lambda_{\mathrm{co}}(F):\mathsf{in}(\lambda)\subsetneq\mathsf{in}(\lambda')\}\) definiert. 
    Die Menge der \emph{stable}-Semantiken von F ist durch \(\Lambda_{\mathrm{st}}(F)=\{\lambda\in\Lambda_{\mathrm{co}}(F)\mid\mathsf{und}(\lambda)=\emptyset\}\) definiert.
\end{Definition}
...
\begin{Definition}
    ...
    Seien \(V, X, Y, Z\) Mengen und \(X,Y,Z \subseteq V\). Ein \emph{Abhängigkeitsmodell} \(I\) über \(V\)
