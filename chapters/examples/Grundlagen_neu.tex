...
\begin{Definition}
    ...
    \begin{itemize}
        \item \(\lambda\) ist ein \emph{Complete-Labelling}, das heißt
        \(\lambda\in\Lambda_{\mathsf{co}}(F)\), genau dann, wenn
        \(\lambda\) vollständig ist.

        \item \(\lambda\) ist ein \emph{Grounded-Labelling}, das heißt
        \(\lambda\in\Lambda_{\mathsf{gr}}(F)\), genau dann, wenn
        ...
        \item \(\lambda\) ist ein \emph{Preferred-Labelling}, das heißt
        \(\lambda\in\Lambda_{\mathsf{pr}}(F)\), genau dann, wenn
        \(\lambda\) vollständig ist und \(\mathsf{in}(\lambda)\)
        bezüglich \(\subseteq\) maximal unter den
        \(\mathsf{in}\)-Mengen aller vollständigen Labellings von \(F\)
        ist.

        \item \(\lambda\) ist ein \emph{Stable-Labelling}, das heißt
        \(\lambda\in\Lambda_{\mathsf{st}}(F)\), genau dann, wenn
...
\begin{Definition}
    ...
    \]
    Insbesondere gilt \(\lambda_1|_X=\lambda_2|_X\) genau dann, wenn
...
Die folgende abstrakte Darstellung der bedingten Unabhängigkeit orientiert
...
Die Bedingungsmenge hält den betrachteten Kontext fest. Eine
Unabhängigkeitsaussage bringt zum Ausdruck, dass sich die Beziehung zwischen
den beiden übrigen Mengen unter diesem festgehaltenen Kontext trennen lässt.
...
Die Symmetrie macht die Reihenfolge der beiden betrachteten Mengen
unerheblich. Die Dekomposition überträgt eine Aussage über eine Vereinigung
auf deren Teilmengen. Die schwache Vereinigung erlaubt es, einen Teil der
zweiten Menge in die Bedingungsmenge zu verschieben. Die Kontraktion führt
zwei passende Aussagen zu einer Aussage über die Vereinigung zusammen.
...
\textcite{egly:2006:reasoning-qbf}.
...
\begin{Definition}
    ...
    Sei nun \(\nu\) eine Belegung von
    \(\operatorname{FV}(\Phi)\setminus X\), und sei
    \(\beta\colon X\to\{0,1\}\) eine Belegung von \(X\). Mit
    \(\nu\cup\beta\) bezeichnen wir die gemeinsame Belegung auf
    \(\operatorname{FV}(\Phi)\). Dann gilt
    \begin{align*}
        \nu\models\forall X\,\Phi
        &\quad\Longleftrightarrow\quad
        \forall\beta\colon X\to\{0,1\}\colon
        \nu\cup\beta\models\Phi,\\
        \nu\models\exists X\,\Phi
        &\quad\Longleftrightarrow\quad
        \exists\beta\colon X\to\{0,1\}\colon
        \nu\cup\beta\models\Phi.
    \end{align*}
\end{Definition}

Die Formel \(\forall X\,\exists Y\,\varphi\) verlangt somit, dass zu jeder
Belegung von \(X\) mindestens eine Belegung von \(Y\) existiert, unter der
\(\varphi\) wahr ist. Die Belegung von \(Y\) darf dabei von der zuvor
gewählten Belegung von \(X\) abhängen. Daher ist die Reihenfolge
unterschiedlicher Quantoren im Allgemeinen wesentlich.
...
\begin{Definition}
    ...
    \emph{Quantorenblock}. Der Wechsel von \(Q_i\) zu \(Q_{i+1}\) heißt
    \emph{Quantorenalternation}; eine solche QBF besitzt folglich
    \(m-1\) Quantorenalternationen.
