...
\(\Lambda_{\mathrm{st}}(F)\) kann hingegen leer sein.
Im Folgenden verwenden wir
\(\mathrm{gr}\), \(\mathrm{co}\), \(\mathrm{pr}\) und \(\mathrm{st}\)
als Kürzel für die Semantiken
\emph{grounded}, \emph{complete}, \emph{preferred}
beziehungsweise \emph{stable}.

\begin{Definition}[Extensionen und Extensionszahl]
    \label{def:af-extensions}
    Sei \(F=(A,R)\) ein AF und sei
    \(\sigma\in\{\mathrm{gr},\mathrm{co},\mathrm{st},\mathrm{pr}\}\).
    Eine Menge \(E\subseteq A\) heißt \emph{\(\sigma\)-Extension} von \(F\),
    wenn ein Labelling \(\lambda\in\Lambda_{\sigma}(F)\) mit
    \(E=\mathsf{in}(\lambda)\) existiert.
\end{Definition}
Für die hier betrachteten Semantiken stimmt die Anzahl der 
\(\sigma\)-Extensionen von einem AF \(F\) mit 
\(\lvert\Lambda_{\sigma}(F)\rvert\) überein.
...
Abbildung~\ref{fig:polynomielle-hierarchie} veranschaulicht die so
definierten Klassen und ihre Inklusionsbeziehungen innerhalb der
polynomiellen Hierarchie. Insbesondere sind
\(\Sigma_k^{\mathrm{P}}\) und \(\Pi_k^{\mathrm{P}}\) jeweils in
\(\Delta_{k+1}^{\mathrm{P}}\) enthalten.

\begin{figure}[htbp]
    \centering
    \input{gfx/polynomielle_hierarchie.tex}
...
\begin{Definition}[Entscheidungsprobleme für AFs]
    ...
    Sei \(F=(A,R)\) ein AF und sei
    \(\sigma\in
    \{\mathrm{gr},\mathrm{co},\mathrm{st},\mathrm{pr}\}\).
...
\begin{table}[h]
    ...
    Tabelle adaptiert nach \cite{dvorak:2012:computational-aspects}.}
